...
| `name`          | `string`   | Name of the plugin, e.g. `key-auth`.
| `created_at`    | `number`   | Creation time (seconds from epoch).
...
| `config`        | `table`    | Plugin configuration schema.
| `run_on`        | `string`   | Determines on which node the plugin should run on service mesh.
| `protocols`     | `table`    | The plugin will run on specified protocols.
...
is be a valid configuration property for your plugin, describing the rules for that
...
There are many more, but the above are quite commonly used.
